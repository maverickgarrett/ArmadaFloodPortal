...
(function (ExpensePage) {
    ...
    ExpensePage[ExpensePage["orders"] = 2] = "orders";
...
function getCurrentPage() {
    ...
    } else if ((window.location.hash === '#/' + ExpensePage[ExpensePage.orders])) {
...
crossroads.addRoute('/:lang:', function () {
    ...
    if ((currentPage && currentPage !== '') || (window.location.hash === '#/' + getCurrentPage())) {
        ...
        var ajaxHTML = new ej.base.Ajax('/SyncOrder/' + currentPage.charAt(0).toUpperCase() + currentPage.slice(1), 'GET', true);
...
}).rules = { lang: ['dashboard', 'about', 'orders'] };
crossroads.bypassed.add(function (request) {
    var samplePath = ['dashboard', 'about', 'orders'];
